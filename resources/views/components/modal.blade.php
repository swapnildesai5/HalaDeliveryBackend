<div x-cloak x-show="open" class="fixed inset-0 z-20 overflow-y-auto">
    <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div @if ( $clickAway ?? true ) @click.away="open = false" @endif class="relative inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            {{-- close model  --}}
            ...
            <form wire:submit.prevent="{{ $action ?? '' }}">
                ...
                <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    @endif
                    <button type="button" class="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @if ( $onCancel ?? false ) wire:click="{{ $onCancel }}" @else wire:click="$emitUp('dismissModal')" @endif>
                        {{ $cancelText ?? __('Close') }}
                    </button>
                </div>
                @if($withForm ?? true)
