...
<div>
    ...
    <x-baseview title="{{ __('Users') }}">
        <div class="flex items-center my-4 gap-x-4">
            <div class="cursor-pointer px-4 py-2 {{ $selectRole == '' ? 'text-white bg-gray-500':'text-black bg-gray-300' }} rounded" wire:click="sortList('')">{{ __('All') }}</div>
            @foreach($roles as $role)
                <div class="cursor-pointer px-4 py-2 {{ $selectRole == $role->name ? 'text-white bg-gray-500':'text-black bg-gray-300' }}  rounded" wire:click="sortList('{{ $role->name }}')">{{ __(ucfirst($role->name)) }}</div>
            @endforeach
        </div>
        ...
        <x-modal>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <x-details.item title="{{ __('Wallet') }}"
                    text="{{ currencyFormat($selectedModel->wallet->balance ?? 0.00) }}" />
