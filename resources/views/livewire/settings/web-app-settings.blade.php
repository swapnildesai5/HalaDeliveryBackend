<div>
    ...
    <x-baseview title="{{ __('Web App Settings') }}">
        ...
        <x-form action="saveAppSettings" backPressed="$set('showApp', false)">
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div class="">
                    ...
                    {{-- language selector --}}
                    <x-select title="{{ __('Language') }}" :options='$languages' name="locale" selected="{{ $locale }}" :defer="true" />
                    <div>
                        <x-input title="{{ __('Website Color') }}" name="websiteColor" type="color" class="h-10" />
                        <p class="text-sm text-gray-500">{{ __('Note: You will need to refresh the page for the color to take effect')}}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ __('Note: For example if you want to allow phone from Ghana you enter')}}
                            GH</p>
                        <p class="text-sm text-gray-500">{{ __('Note: if you want to allow phone from any country enter') }} AUTO
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">{{ __("The max number of days customers are allowed to schedule an order")}}</p>
                        </div>
                        {{-- schedule order time --}}
                        <div>
                            <x-input title="{{ __('Schedule Order Time') }}" name="minScheduledTime" type="number" />
                            <p class="mt-1 text-xs text-gray-500">{{ __("Hours before pickup time for system to move order from 'scheduled' to 'pending'")}}</p>
                        </div>
                        {{-- schedule order time --}}
                        <div>
                            <x-input title="{{ __('Max Schedule Time') }}" name="maxScheduledTime" type="number" />
                            <p class="mt-1 text-xs text-gray-500">{{ __("Hours from close time that customer still allow to book for")}}</p>
                        </div>
                ...
                <div>
                    ...
                    <div class="flex items-center my-5 space-x-10">
                        ...
                        <x-input title="{{ __('Register Image') }}" name="registerImage" :defer="false" type="file" />
                    </div>
